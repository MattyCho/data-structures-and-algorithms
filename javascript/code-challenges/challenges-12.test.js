...
const replaceZeros = (string) => {
  let newString = /0/g
  return string.replace(newString, 'zero');
}
...
const validatePin = (pin) => {
  let pinCheck = /^[0-9][0-9][0-9][0-9]$/g;
...
const validateWord = (word) => {
  let wordCheck = /^[A-Za-z]{5,10}$/g;
  return wordCheck.test(word);
};
...
const validateEmail = (email) => {
  // let emailCheck = /^\S.*@\S+.$/;
  let emailCheck = /^[\w]+[a-zA-Z0-9.]+[\w]@[/w]+\.[\w]{3,3}$/
...
const validatePhoneNumber = (phoneNumber) => {
  let phoneCheck = /^(\()?\d{3}(\))?(-|\s)?\d{3}(-|\s)\d{4}$/
